...
import { ShoppingBag, ChevronDown, Phone, Calendar, Clock, Package } from 'lucide-react';
    ...
    <div className="bg-slate-800/50 border border-slate-700/50 rounded-2xl overflow-hidden transition-all">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 text-slate-500 text-xs mt-0.5">
              ...
              <span className="flex items-center gap-1"><Calendar className="w-3 h-3" />{pedido.fecha}</span>
              <span className="flex items-center gap-1"><Clock className="w-3 h-3" />{pedido.hora}</span>
            </div>
          </div>
        </div>
      ...
      {open && (
        <div className="border-t border-slate-700/50 p-4 space-y-4">
              ...
              {pedido.productos.map((item, i) => (
                <div key={i} className="flex items-center justify-between text-sm">
                  <span className="text-slate-300">
                    {item.nombre}
                    {item.cantidad > 1 && <span className="text-slate-500 ml-1">x{item.cantidad}</span>}
                  </span>
                  <span className="text-slate-400">{fmt(item.precio * (item.cantidad || 1))}</span>
                </div>
              ))}
          ...
          {pedido.notas && (
            <div>
              <p className="text-slate-500 text-xs font-medium uppercase tracking-wide mb-1">Notas</p>
